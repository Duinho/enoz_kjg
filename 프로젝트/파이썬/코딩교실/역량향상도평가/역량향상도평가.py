...
def 자동화():
    ...
    for i in range(len(col_n)):  # col_n의 저장된 개수만큼 반복
        page.goto(col_l[i])  # 해당 학생의 링크로 이동
        page.wait_for_selector('span.NPEfkd.RveJvd.snByac', timeout=10000)  # 제출 버튼이 나올 때까지 기다림
        page.fill('input[aria-labelledby="i1"]', str(col_n[i]))  # 이름란에 이름 기입
        page.fill('input[aria-labelledby="i5"]', str(col_s[i]))  # 학교란에 학교 기입
        if col_g[i] > 6:  # 만약 6보다 크다면 (중학생이라면)
            col_g[i] = col_g[i] - 6  # 6을 빼고 저장(중1은 7이 저장되어 있으므로 6을 빼서 1로 바꿈)
        page.fill('input[aria-labelledby="i9"]', str(f'{col_g[i]}학년'))  # 학년란에 학년 기입
