...
browser = None
col_9 = None
만족도조사링크 = 'https://forms.gle/238VncFmbA8U1rG88'                    # 그 때 그 때 맞는 링크로 수정해야 함
시력인원 = 100    # 명수 수정
심폐인원 = 200    # 명수 수정 비만은 나머지 인원으로 됨
...
async def 자동화():
    ...
    for i in range(len(col_9)):                                                  # col_9에 저장된 개수만큼 반복
        ...
        for j in range(1, len(col_a)):                                          
            col_a[j] += (5*j)                                                      # col_a에 0 5 10 15로 순차적으로 더함(5개 항목) 여기에 1번항목의 문항을 스킵해야함 그래서 +3을 함 
        ...
        for k in range(0,8):                                                     # 8번 반복함 (총 8문제)
            await elements[3+col_a[k]-1].click()                                   # 클릭 항목을 순차적으로 체크함
    
        await page.fill('textarea[aria-labelledby="i46"]', str(col_9[i]))        # 마지막 주관식에 엑셀에 col_9에 있는 값을 적음
...
async def 동작():
    global browser, page, 시력인원, 심폐인원  # 전역 변수로 사용
    await 엑셀()
