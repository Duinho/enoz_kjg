...
Adafruit_NeoPixel strip(12, 8, NEO_GRB + NEO_KHZ800);
...
void NEO_C() {
  ...
  if (NEO_1 == 'B' && NEO_2 == 'R' && NEO_3 == 'G' && NEO_4 == 'O') {
    digitalWhite(OUT, HIGH);
    BUT_C = 0;
  } else {
    digitalWhite(OUT, LOW);
  }
}

void NEO_V(int i, char j) {
  if (j == 'R') strip.setPixelColor(i, BRIGHTNESS, 0, 0);
  if (j == 'G') strip.setPixelColor(i, 0, BRIGHTNESS, 0);
  if (j == 'B') strip.setPixelColor(i, 0, 0, BRIGHTNESS);
  if (j == 'O') strip.setPixelColor(i, BRIGHTNESS, BRIGHTNESS / 1.82, 0);
