...
import 만족도조사
import 사전역량조사
import 이룸캠프만족도      # 이룸캠프 만족도 자동화 모듈
...
frame6 = tk.LabelFrame(root, text="이룸캠프 만족도", font=("Arial", 14), padx=10, pady=10)
...
btn_noc_download = tk.Button(
    frame1, text="양식 엑셀 다운받기", width=30, height=2,
    command=이름O처리.download_template_noc
)
btn_noc_upload = tk.Button(
    frame1, text="엑셀 파일 올리기", width=30, height=2,
    command=noc_excel
)
...
def perform_yl():
    ...
    if filepath:
        error_rate = simpledialog.askfloat(
            "오답률 설정",
            "문항당 오답률 입력 (예: 0.008)",
            initialvalue=0.008,
            minvalue=0.0,
...
btn_yl_download = tk.Button(
    frame2, text="역량향상도평가 양식 다운받기",
    width=30, height=2,
    command=역량향상도평가.download_template_yl
)
btn_yl_auto = tk.Button(
    frame2, text="역량향상도 평가 자동화 실행",
    width=30, height=2,
    command=perform_yl
)
...
lbl_note = tk.Label(
    frame3,
    text="※ 9번 문항 필수 체크를 해제한 후 실행해주세요.",
    fg="red",
    ...
    if filepath:
        if messagebox.askyesno("확인", "선택한 엑셀 파일을 사용하여 사전역량조사를 진행하시겠습니까?"):
            ...
            if not 저장_경로:
                return
            try:
                사전역량조사.동작(filepath, 저장_경로)
                messagebox.showinfo("완료", "사전역량조사가 완료되었습니다!")
            except Exception as e:
                messagebox.showerror("오류", f"사전역량조사 중 오류가 발생했습니다:\n{str(e)}")


btn_sajeon_download = tk.Button(
    frame4,
    text="사전역량조사 양식 다운받기",
    width=30, height=2,
    command=사전역량조사.download_template_sajeon
)
btn_sajeon_upload = tk.Button(
    frame4,
    text="사전역량조사 자동화 실행",
    width=30, height=2,
    command=sajeon_excel
)
...
def run_iloom_satisfaction():
    ...
    try:
        이룸캠프만족도.run_from_excel(filepath)
        messagebox.showinfo("완료", "이룸캠프 만족도 설문 자동화가 완료되었습니다.")
    except Exception as e:
        messagebox.showerror("오류", f"이룸캠프 만족도 자동화 중 오류가 발생했습니다:\n{e}")


btn_iloom_download = tk.Button(
    frame6,
    text="이룸캠프 만족도 양식 다운받기",
    width=30, height=2,
    command=이룸캠프만족도.download_template_iloom
)
btn_iloom_run = tk.Button(
    frame6,
    text="이룸캠프 만족도 자동화 실행",
    width=30, height=2,
    command=run_iloom_satisfaction
)
btn_iloom_download.pack(pady=15)
btn_iloom_run.pack(pady=15)
